...
const dateEnde = new Date("2023-01-20"); //dies muss nachher noch global festgelegt und übergeben werden. Zur einfacheren Handhabung lege ich es hier fest.
...
//     date
//   ) => {
//     const [data1, setData1] = useState({ dates: [], sums: [] });
//     const [data, setData] = useState({ dates: [], sums: [] });
...
//         const dateEndePosition = filteredData.findIndex((d) => d.date.getTime() === dateEnde.getTime());
...
//           if (filteredData[i].date >= dateAnfang && filteredData[i].date <= dateEnde) {
//             let sum = 0;
//             for (let j = i; j < i + 7; j++) {
//               for (let k = 0; k <= 80; k++) {
...
//       // hier kann ich functionen auf filteredData anwenden
...
export const useDataDatenstand = (
  ...
  const [data, setData] = useState(null);
  const [datesAfterEnde, setDatesAfterEnde] = useState(0);
